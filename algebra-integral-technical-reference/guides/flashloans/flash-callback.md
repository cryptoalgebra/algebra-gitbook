...
Assign local variables of type `address` as `deployer,` `token0` and `token1` to allow the router to interact with tokens from flash.

```solidity
        address token0 = decoded.poolKey.token0;
        address token1 = decoded.poolKey.token1;
        address deployer = decoded.poolKey.deployer;
