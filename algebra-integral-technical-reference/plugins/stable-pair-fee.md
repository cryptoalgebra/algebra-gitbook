...
The purpose of the Stable Pair Fee Plugin is to price swaps in pools whose two assets are expected to hold the same value (USDC/USDT), without reading an external oracle at swap time.
...
The spread the trader sees is constant; only its split between price and fee changes.
...
The plugin stores three values per pool, written on the first swap of each block: the pool price at the start of the block, the current decaying fee, and the block number. Later swaps in the same block are priced from the stored price, so splitting one large swap into several gives no fee advantage.

Outside the corridor, swaps toward the reference price pay the decaying fee and swaps away from it are free. On the first swap of a new block, the decay starts from:

* **The far boundary fee**, if the price just left the corridor or jumped across the reference price. This is a new deviation and gets maximum protection.
* **The previous fee**, otherwise, adjusted for the price movement so the arbitrage leg never gets a better pre-impact price. It is raised if the pool drifted further away and capped at the far boundary fee if it moved back toward the reference price.

The starting fee is never below the target fee, and decay counts every block since the last write. Returning inside the corridor drops the decaying fee, so the next exit starts over at the far boundary fee. Updating the configuration resets the state the same way.
...
The plugin has a single admin role: `ALGEBRA_BASE_PLUGIN_MANAGER` can call `setStableFeeConfig(...)`. Configuration is a per-pool decision - each pool keeps its own reference price and fee settings in its own plugin.
