...
_Developer note: Mint the specified amount of token to the specified address and freeze it until the specified date. Be careful, gas usage is not deterministic, and depends on how many freezes \_to address already has._
...
_Developer note: release first available freezing tokens._

### getFreezing

`function getFreezing(address _addr, uint256 _index) external view returns (uint64 _release, uint256 _balance)` view external

_Developer note: gets freezing end date and freezing balance for the freezing portion specified by index._
