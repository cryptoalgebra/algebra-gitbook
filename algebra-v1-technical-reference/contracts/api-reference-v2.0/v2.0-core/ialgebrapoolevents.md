...
Emitted exactly once by a pool when #initialize is first called on the pool _Developer note: Mint/Burn/Swap cannot be emitted by the pool before Initialize_
...
Emitted when liquidity is minted for a given position _Developer note: If the top and bottom ticks match, this should be treated as a limit order_
...
Emitted when fees are collected by the owner of a position _Developer note: Collect events may be emitted with zero amount0 and amount1 when the caller chooses not to collect fees_
...
Emitted when a position's liquidity is removed _Developer note: Does not withdraw any fees earned by the liquidity position, which must be withdrawn via #collect_
...
Emitted in case of an error when trying to write to the DataStorage _Developer note: This shouldn't happen_
